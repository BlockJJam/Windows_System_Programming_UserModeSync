...
BOOL MyInitOnceExecuteOnce(PMY_INIT_ONCE plo, PMY_INIT_ONCE_FN pfnInit, PVOID pPrm, PVOID* ppCtx)
{
	BOOL bIsOK = TRUE;

	EnterCriticalSection(&plo->_cs);
	if (plo->_ctx == NULL)
	{
		bIsOK = pfnInit(plo, pPrm, ppCtx);
		if (bIsOK)
			plo->_ctx = *ppCtx;
	}
	else
		*ppCtx = plo->_ctx;
	LeaveCriticalSection(&plo->_cs);
