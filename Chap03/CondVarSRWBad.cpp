...
struct WORK_ENV
{
	VAR_QUEUE			_queue;
	SRWLOCK				_srwQue;
	CONDITION_VARIABLE	_cvNE;
	CONDITION_VARIABLE	_cvNF;
};
...
DWORD WINAPI ProducerProc(PVOID pParam)
{
	...
	printf("== Producer Thread %d termiated ... \n", dwThrId);
	return 0;
}
...
DWORD WINAPI ConsumerProc(PVOID pParam)
{
	...
	while (true)
	{
		AcquireSRWLockShared(&pwe->_srwQue);
		{
			...
			{
				SleepConditionVariableSRW(&pwe->_cvNE, &pwe->_srwQue, INFINITE, CONDITION_VARIABLE_LOCKMODE_SHARED);
			}
